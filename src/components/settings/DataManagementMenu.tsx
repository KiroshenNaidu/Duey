...
// Modern report styling, matching the History page's PDF exports: charcoal table
// heads, hairline row separators instead of full cell grids, accent-tinted totals,
// and a timestamped footer.
// ──────────────────────────────────────────────────────────────────────────────

const PDF_INK = [26, 28, 32] as const;
const PDF_CHARCOAL = [45, 50, 58] as const;
const PDF_ACCENT = [151, 223, 104] as const;    // Duey primary (lime) as RGB
const PDF_ACCENT_DARK = [86, 140, 50] as const;
const PDF_ACCENT_TINT = [239, 249, 231] as const;
...
function drawTable(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Hairline separator under each row — lighter than a full cell grid.
  ...
  if (totalRow) {
    if (y + HDR_H > PB) { doc.addPage(); y = 15; }
    doc.setFillColor(...PDF_ACCENT_TINT);
    doc.rect(LEFT, y, totalWidth, HDR_H, 'F');
...
function drawPageFooter(doc: any, pageNum: number, totalPages: number): void {
  ...
  doc.text(`Generated by Duey · ${stamp}`, W - 10, footerY, { align: 'right' });
}
...
export function DataManagementMenu() {
  ...
  const exportAsPdf = async (): Promise<Blob> => {
      ...
      const sectionHeader = (title: string) => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.setTextColor(...PDF_CHARCOAL);
        doc.text(title, 10, y);
        // Accent underline keyed to the title width — matches the History page reports.
        doc.setDrawColor(...PDF_ACCENT);
        doc.setLineWidth(0.7);
        doc.line(10, y + 1.7, 10 + doc.getTextWidth(title), y + 1.7);
        y += 5;
      };
      ...
      <Card>
        <CardContent className="p-3 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={() => runExport('history', 'pdf', exportAsPdf)}   variant="secondary" size="sm" className="justify-start gap-2 text-xs">
